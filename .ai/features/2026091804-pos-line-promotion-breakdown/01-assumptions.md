...
| A-09 | Cộng dồn giảm tay + CTKM: engine đã tính % trên phần **sau** giảm tay (`discount-math.ts:11`: `qty*unitPrice − manualLineDiscount`), nên UI chỉ trừ tiếp `discountAmount` engine trả — không tự tính % lại. Ví dụ SKU-685 giảm tay 50.000 + CTKM-A 10% → 63.500, *Thành tiền* 571.500 | high | no | Nếu UI tự tính % → lệch với `Còn phải thu`; AC-04 bắt | confirmed | `discount-math.ts` |
