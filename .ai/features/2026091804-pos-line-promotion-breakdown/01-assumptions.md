...
| A-04 | Nút ✕ trên dòng *Khuyến mại* (panel) từ nay chỉ loại các CTKM **hóa đơn** đang áp — vì dòng đó chỉ còn đại diện cho chúng. CTKM hàng hóa muốn bỏ thì vào modal *Chương trình khuyến mãi* (UOW-09 cũ đã cho tick/bỏ tick) | medium | no | Nếu Akenzy muốn ✕ vẫn "bỏ tất cả" như hiện nay: đổi một handler + câu xác nhận, không lan | confirmed | Làm theo giả định (ADR-05, T-01-03): ✕ chỉ loại CTKM hóa đơn, hộp xác nhận nêu đích danh — AC-10 có ảnh `L-04`/`L-05`. Akenzy được nêu điểm này hai lần (tóm tắt plan trước G3 và khi pass G3) và không phản đối; nếu muốn "bỏ tất cả" thì là một handler, ghi ở PR-DRAFT mục "Hành vi đổi" |
...
| A-11 | `modules/mobile` và backoffice-web đọc `GET /invoices/:id` (nếu có) chỉ **thêm** field, không mất gì; không sửa hai app đó | medium | no | Nếu một client validate strict schema: 400 khi nhận field lạ — kiểm bằng `grep appliedPromotions` ngoài pos-web | confirmed | `grep -rln appliedPromotions` ngoài pos-web (T-03-01): chỉ `mobile-manager-invoice.service.ts` (`.map(p => p.type)`) và `packages/api-client` generated; backoffice-web không dùng. Spec mobile vẫn xanh; api-client regen chỉ thêm |
