 ...
 * receipt posts DR cash / CR contra, where the contra account is resolved from
...
> = {
  [CashReceiptPurpose.OTHER]: AccountingDefaultAccountRole.OTHER_INCOME,
...
@Injectable()
export class CashReceiptsService implements OnModuleInit {
   ...
   */
  onModuleInit(): void {
    this.mediaReaders.register(MediaOwnerType.CASH_RECEIPT, (id, actor) =>
  ...
  /**
   * Create a manual cash receipt and post it to the ledger immediately (no DRAFT
   * stage). The contra account is resolved server-side from the voucher purpose
   * (DR cash / CR contra); an explicit `contraAccountId` is honoured only as an
   * override. Movement + journal entry + POSTED voucher are written atomically.
  ...
  async create(
    ...
    const purpose = dto.purpose ?? CashReceiptPurpose.OTHER;
    ...
    return this.dataSource.transaction(async (manager) => {
      ...
      // conflict — before `createAndPostInternalInTx` burns a PT number and
      ...
      await manager.update(CashReceiptEntity, createdId, { attachmentIds });
  ...
  async reverse(
    ...
    return this.dataSource.transaction(async (manager) => {
      ...
      if (original.referenceType === CashReceiptReferenceType.INVOICE_DEBT) {
  ...
  private async insertPostedVoucher(
    ...
    const voucher = manager.create(CashReceiptEntity, {
      ...
      // minted (see `CashReceiptCreateAndPostArgs.id`); every other caller
  ...
  > {
    ...
    const attachments: VoucherAttachment[] = (
      attachmentsByOwner.get(receipt.id) ?? []
    ).map((m) => ({
      id: m.id,
      fileName: m.fileName,
    ...
    return Object.assign(receipt, { sourceLink, linkedVoucher, attachments });
  }
