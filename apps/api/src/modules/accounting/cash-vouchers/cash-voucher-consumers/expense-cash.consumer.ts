...
/** Expense paid in cash → Phiếu chi (EXPENSE), linking movement + JE. */
@Injectable()
export class ExpenseCashConsumer {
  private readonly logger = new Logger(ExpenseCashConsumer.name);
  ...
  @OnDomainEvent(ERP_TOPICS.CASH_VOUCHER_NEEDED_EXPENSE)
  async handle(event: DomainEvent<CashVoucherNeededPayload>): Promise<void> {
    ...
    this.logger.log(
      `Expense ${p.sourceId} → ${result.voucherNumber} (payment=${result.voucherId})`,
    );
  }
}
