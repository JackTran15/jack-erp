...
describe("SearchItemCategoryTreeHandler", () => {
  ...
  it("keeps full hierarchy for deep matching descendants", async () => {
    const handler = handlerWith([
      { id: "p1", name: "Giày", code: "GIAY" },
      { id: "c1", name: "Giày nữ", parentGroupId: "p1", code: "GIAYNU" },
      {
        id: "g1",
      ...
      { id: "c2", name: "Giày nam", parentGroupId: "p1", code: "GIAYNAM" },
    ]);

    const { data } = await handler.execute(
      new SearchItemCategoryTreeQuery({ search: "cao" }, actor),
    );

    expect(data.map((n) => n.id)).toEqual(["p1"]);
    expect(data[0].children.map((n) => n.id)).toEqual(["c1"]);
    expect(data[0].children[0].children.map((n) => n.id)).toEqual(["g1"]);
  });
});
