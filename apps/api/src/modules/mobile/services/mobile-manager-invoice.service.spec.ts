...
import { MobileManagerInvoiceService } from './mobile-manager-invoice.service';
 ...
 */
describe('MobileManagerInvoiceService', () => {
  let service: MobileManagerInvoiceService;
  let query: jest.Mock;
  ...
  beforeEach(async () => {
    ...
    service = module.get(MobileManagerInvoiceService);
  });

  const run = (
    overrides: Partial<Parameters<MobileManagerInvoiceService['list']>[0]> = {},
  ) =>
    service.list(
      {
        page: 1,
        limit: 20,
        order: MobileManagerInvoiceOrder.DESC,
        dateBasis: MobileManagerInvoiceDateBasis.CREATED,
        ...overrides,
      },
      actor,
    );
  ...
  describe('list', () => {
    ...
    it('trạng thái app -> các giá trị cột (unpaid gom debt + partial_debt)', async () => {
      await run({
        status: [MobileManagerInvoiceStatus.UNPAID, MobileManagerInvoiceStatus.CANCELLED],
      });
    ...
    describe('phạm vi chi nhánh — đúng tập phân công, không có vế hợp nhất', () => {
      ...
      it('không phân công → 403', async () => {
        await expect(
          service.list(
            { page: 1, limit: 20, order: MobileManagerInvoiceOrder.DESC, dateBasis: MobileManagerInvoiceDateBasis.CREATED },
            { ...actor, branchIds: [] },
          ),
    ...
    it('dateBasis chọn cột cho CẢ khoảng lọc lẫn ORDER BY; `to` bao trọn ngày cuối', async () => {
      await run({
        ...
        from: '2026-08-01',
        to: '2026-08-31',
        order: MobileManagerInvoiceOrder.ASC,
      });
