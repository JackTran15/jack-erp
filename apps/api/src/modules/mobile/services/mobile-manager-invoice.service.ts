 ...
 */
const STATUS_OF: Partial<Record<InvoiceStatus, MobileManagerInvoiceStatus>> = {
  [InvoiceStatus.PAID]: MobileManagerInvoiceStatus.PAID,
  [InvoiceStatus.DEBT]: MobileManagerInvoiceStatus.UNPAID,
  [InvoiceStatus.PARTIAL_DEBT]: MobileManagerInvoiceStatus.UNPAID,
  [InvoiceStatus.CANCELLED]: MobileManagerInvoiceStatus.CANCELLED,
};

/** Chiều ngược của [STATUS_OF]: một trạng thái app -> các giá trị cột. */
const STATUS_COLUMN_VALUES: Record<MobileManagerInvoiceStatus, InvoiceStatus[]> = {
  [MobileManagerInvoiceStatus.PAID]: [InvoiceStatus.PAID],
  [MobileManagerInvoiceStatus.UNPAID]: [InvoiceStatus.DEBT, InvoiceStatus.PARTIAL_DEBT],
  [MobileManagerInvoiceStatus.CANCELLED]: [InvoiceStatus.CANCELLED],
};
...
const TYPE_OF: Record<InvoiceType, MobileManagerInvoiceResponseDto['type']> = {
 ...
 */
const DATE_COLUMN: Record<MobileManagerInvoiceDateBasis, string> = {
  [MobileManagerInvoiceDateBasis.CREATED]: 'i.created_at',
  [MobileManagerInvoiceDateBasis.ISSUED]: 'i.issued_at',
};

const ORDER_DIRECTION: Record<MobileManagerInvoiceOrder, 'ASC' | 'DESC'> = {
  [MobileManagerInvoiceOrder.ASC]: 'ASC',
  [MobileManagerInvoiceOrder.DESC]: 'DESC',
};
 ...
 */
@Injectable()
export class MobileManagerInvoiceService {
  constructor(
  ...
  async list(
    query: {
      ...
      page: number;
      limit: number;
      order: MobileManagerInvoiceOrder;
  ...
  async findById(
    ...
    const lines: MobileManagerInvoiceLineDto[] = invoice.items
...
function toMobileInvoice(row: RawRow): MobileManagerInvoiceResponseDto {
  return {
    id: row.id,
    code: row.code,
    ...
    status: STATUS_OF[row.status] ?? MobileManagerInvoiceStatus.UNPAID,
