...
describe('ItemDiscountStrategy', () => {
  ...
  it('treats a program with no group as not applicable instead of throwing (QA #8)', () => {
    const emptyGroupsProgram = aProgram().ofType(PromotionProgramType.ITEM_DISCOUNT)
      .with({ discountMode: undefined, discountValue: undefined })
      .withGroups([aGroup({ lines: [aLine({ discountMode: PromotionDiscountMode.PERCENT, discountValue: 10 })] })])
      .build();
