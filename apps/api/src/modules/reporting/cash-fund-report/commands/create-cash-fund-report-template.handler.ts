...
} from '../cash-fund-report-template.columns.util';
import { ReportRegistry } from '../report-definition';
import { CreateCashFundReportTemplateCommand } from './create-cash-fund-report-template.command';

@CommandHandler(CreateCashFundReportTemplateCommand)
export class CreateCashFundReportTemplateHandler
  implements ICommandHandler<CreateCashFundReportTemplateCommand>
{
  constructor(
  ...
  async execute({
    dto,
    actor,
  }: CreateCashFundReportTemplateCommand): Promise<InvoiceReportTemplateView> {
