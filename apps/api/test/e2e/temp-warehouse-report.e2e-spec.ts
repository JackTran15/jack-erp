...
describe('Temp-warehouse out-goods report (e2e)', () => {
  ...
  /** Một dòng xuất kho tạm. `createdAt` ghi đè sau khi lưu vì nó là @CreateDateColumn. */
  const addIssueLine = async (opts: {
    itemId: string;
    createdAt: Date;
    invoiceId?: string;
    ...
    const line = await repo.save(
      repo.create({
        ...
        carrierUserId: seed.userId,
        status: opts.invoiceId
  ...
  // Ngày/giờ phải render theo giờ VN. `created_at` là timestamp naive-UTC; mốc
  // 19:00Z chọn cố ý vì nó vắt qua nửa đêm giờ VN, nên sai chiều quy đổi thì
  // lệch cả NGÀY chứ không chỉ giờ. Đây là hồi quy cho một lỗi thật: khi báo cáo
  // còn UNION thêm một nguồn `timestamptz`, cả hai nhánh bị nâng kiểu và biểu
  // thức render đổi overload — trừ 7h thay vì cộng (xem ADR-05).
  it('render ngày/giờ theo giờ VN', async () => {
    const code = 'TZ-SKU';
    const itemId = await createItem(code);
    ...
    expect(data).toHaveLength(1);
    expect(data[0]!.date).toBe('16/08/2026');
    expect(data[0]!.time).toBe('02:00:00');
  });
