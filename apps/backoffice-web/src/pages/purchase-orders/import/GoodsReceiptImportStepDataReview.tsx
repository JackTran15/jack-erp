...
const columns: TableColumn<ReviewRow>[] = [
  { key: "sku", label: "Mã SKU", width: 130, render: (row) => String(row.rawData["Mã SKU"] ?? "") },
  { key: "storage", label: "Kho", width: 150, render: (row) => String(row.rawData["Kho"] ?? "") },
  { key: "location", label: "Vị trí", width: 120, render: (row) => String(row.rawData["Vị trí"] ?? "") },
  { key: "quantity", label: "Số lượng", width: 110, className: "text-right", render: (row) => String(row.rawData["Số lượng"] ?? "") },
  { key: "unitPrice", label: "Đơn giá", width: 130, className: "text-right", render: (row) => Number(row.normalizedData?.unitPrice ?? row.rawData["Đơn giá"] ?? 0).toLocaleString("vi-VN") },
  {
    key: "status",
    label: "Tình trạng",
    width: 340,
    render: (row) => (
      <span className={row.status === ImportRowStatus.ERROR ? "text-destructive" : row.warningMessages?.length ? "text-amber-700" : ""}>
        {row.statusLabel}
      </span>
    ),
  },
  ...
  return (
    <div className="flex min-h-0 flex-1 flex-col gap-4">
      <div className="flex flex-wrap items-center gap-6 text-sm">
        <span>Tổng số <strong>{job.totalRows}</strong></span>
        ...
        {job.errorRows > 0 ? (
          <span>
            Không hợp lệ <strong className="text-destructive">{job.errorRows}</strong>{" "}
            (<button type="button" className="inline-flex items-center gap-1 text-[#2563eb] hover:underline" onClick={() => void handleDownload()}>
