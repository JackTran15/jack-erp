...
// PurchaseOrdersPage.tsx renders through react-query hooks with side effects
// that need a real DOM; this workspace has no jsdom/RTL (see CLAUDE.md), so
// the fetch-on-click behaviour cannot be exercised by rendering the
// component and clicking a button. This test instead pins the exact source
// shape of the helper and the three toolbar actions that use it, so a
// regression (flag re-added, error path opening the dialog anyway, a second
// lock flag) fails CI instead of silently shipping (T-01-02, AC-03/AC-04,
// ADR-02).
...
function extractBlock(startLiteral: string, endLiteral = "},"): string {
  ...
  expect(start, `expected to find ${startLiteral} in PurchaseOrdersPage.tsx`).toBeGreaterThan(-1);
  const end = pageSource.indexOf(endLiteral, start);
  expect(end).toBeGreaterThan(-1);
  return pageSource.slice(start, end);
}

describe("fetchGoodsReceiptWithLines helper", () => {
  it("calls GET /goods-receipts/{id} with no includeLines flag (ADR-01 default stays true)", () => {
    const block = extractBlock(
      "async function fetchGoodsReceiptWithLines(",
      "\n}",
    );
    expect(block).toContain("/goods-receipts/{id}");
    expect(block).not.toContain("includeLines");
  });
});

describe("toolbar actions that need the full line array (AC-03)", () => {
  it("Nhân bản (duplicate) fetches full lines via the shared helper before opening the dialog", () => {
    ...
    expect(block).toContain("fetchGoodsReceiptWithLines(selectedOrder.id)");
  ...
  it("In tem mã (no rows ticked) fetches full lines via the shared helper", () => {
    ...
    // The ticked path is routed through the same helper too.
  ...
  it("Xem (view) does not fetch full lines — the dialog pages them itself", () => {
    ...
    expect(block).not.toContain("fetchGoodsReceiptWithLines(selectedOrder.id)");
    expect(block).toContain("setEditingOrder(selectedOrder)");
  });
});
