...
| 0-5  | `/admin/inventory-item-categories`                   | Tạo category **Điện tử**, **Thực phẩm**, **Thời trang**                                                           |
| 0-6  | `/admin/inventory-items/new`                         | Tạo item đơn: Tên `Laptop`, category: Điện tử, `purchase_price = 15,000,000` → ghi nhớ mã SKU tự sinh (`SKU-001`) |
| 0-7  | `/admin/inventory-items/new`                         | Tạo item đơn: Tên `Bàn phím`, category: Điện tử, `purchase_price = 500,000` → SKU-002                             |
| 0-8  | `/admin/inventory-items/new`                         | Tạo item đơn: Tên `Bánh mì`, category: Thực phẩm, `purchase_price = 20,000` → SKU-003                             |
| 0-9  | `/admin/inventory-items/new`                         | Tạo **hàng hóa có biến thể**: Tên `Áo thun`, category: Thời trang, `purchase_price = 150,000`                     |
|      | Cùng form → **THÔNG TIN THUỘC TÍNH**                 | Thêm thuộc tính **Màu sắc** với 2 giá trị tag: `Đen`, `Trắng`                                                     |
|      | Cùng form → **THÔNG TIN THUỘC TÍNH**                 | Thêm thuộc tính **Size** với 2 giá trị tag: `S`, `M`                                                              |
|      | Cùng form → **DANH SÁCH PHIÊN BẢN**                  | Hệ thống tự sinh 4 variants; điền `Giá mua = 150,000` cho cả 4 dòng                                               |
|      | Click **Lưu**                                        | Tạo xong 1 hàng hóa cha + 4 items con (Đen/S, Đen/M, Trắng/S, Trắng/M)                                            |


> **Lưu ý `purchase_price`:** bắt buộc > 0 cho Report 5, 6, 7 vì dùng làm cost basis.  
> **Lưu ý variant:** để `Tồn kho ban đầu = 0` — tồn kho tạo qua phiếu nhập ở Phase 1 để có ledger entry đầy đủ cho báo cáo. Cột `color` và `size` của mỗi variant dùng để kiểm tra TC-04.
...
**Nhập kho cho variants — Switch sang HN-01**, tạo thêm:


| Bước | Thao tác                                                             |
| ---- | -------------------------------------------------------------------- |
| 1-4  | Tạo phiếu nhập **GR-HN-03** (variant receipt) → tự động POSTED       |
|      | - AT-001-D-S (Áo thun Đen/S): số lượng **30**, đơn giá **150,000**   |
|      | - AT-001-D-M (Áo thun Đen/M): số lượng **25**, đơn giá **150,000**   |
|      | - AT-001-T-S (Áo thun Trắng/S): số lượng **20**, đơn giá **150,000** |
|      | - AT-001-T-M (Áo thun Trắng/M): số lượng **15**, đơn giá **150,000** |


**Switch sang HCM-01**, tạo thêm:


| Bước | Thao tác                                                            |
| ---- | ------------------------------------------------------------------- |
| 1-5  | Tạo phiếu nhập **GR-HCM-02** (variant receipt) → tự động POSTED     |
|      | - AT-001-D-S (Áo thun Đen/S): số lượng **10**, đơn giá **150,000**  |
|      | - AT-001-T-M (Áo thun Trắng/M): số lượng **8**, đơn giá **150,000** |


Sau bước này, `stock_ledger_entries` có thêm 6 rows với `movement_type = PURCHASE_RECEIPT` cho các variants.
...
**Variants của AT-001 (Áo thun):**


| Variant (SKU) | Màu   | Size | Branch | Nhập | Tồn cuối |
| ------------- | ----- | ---- | ------ | ---- | -------- |
| AT-001-D-S    | Đen   | S    | HN-01  | 30   | **30**   |
| AT-001-D-M    | Đen   | M    | HN-01  | 25   | **25**   |
| AT-001-T-S    | Trắng | S    | HN-01  | 20   | **20**   |
| AT-001-T-M    | Trắng | M    | HN-01  | 15   | **15**   |
| AT-001-D-S    | Đen   | S    | HCM-01 | 10   | **10**   |
| AT-001-T-M    | Trắng | M    | HCM-01 | 8    | **8**    |


**Tổng theo mẫu mã AT-001:** HN-01 = 90, HCM-01 = 18, toàn hệ thống = **108**
...
**TC-04: Variant hiện đúng cột parentSku / parentName / color / size**

Điều kiện: Phase 0b + Phase 1 (bước 1-4) đã thực hiện.
...
| Period        | `this_month` |
| Cửa hàng      | HN-01        |
| Thống kê theo | **Hàng hóa** |


Kỳ vọng: 4 rows riêng biệt cho 4 variants của AT-001, mỗi row phải có:


| Cột           | Giá trị kỳ vọng                             |
| ------------- | ------------------------------------------- |
| Mã SKU        | `AT-001-D-S` (hoặc D-M, T-S, T-M tương ứng) |
| Mã SKU mẫu mã | `AT-001`                                    |
| Tên Mẫu mã    | `Áo thun`                                   |
| Màu sắc       | `Đen` hoặc `Trắng`                          |
| Size          | `S` hoặc `M`                                |
| Nhập kỳ       | 30 / 25 / 20 / 15 tương ứng                 |


Nếu cột `Mã SKU mẫu mã` trống hoặc `Màu sắc`/`Size` trống → report chưa join đúng bảng attributes.

---

**TC-04b: Thống kê theo "Mẫu mã" gộp variants lại**
...
Kỳ vọng: AT-001 hiện **1 row duy nhất** (không phải 4 rows), với:

- Nhập kỳ = 30 + 25 + 20 + 15 = **90**
- Tồn cuối = **90**

Các item đơn giản (SKU-001, SKU-002, SKU-003) vẫn hiện 1 row mỗi cái (chúng không có parent nên tự là mẫu mã của mình).

---

**TC-04c: Thống kê theo "Nhóm hàng hóa" gộp theo category**
...
Kỳ vọng: 3 rows (1 row / category):


| Nhóm       | Nhập kỳ                              |
| ---------- | ------------------------------------ |
| Điện tử    | 10 (SKU-001) + 20 (SKU-002) = **30** |
| Thực phẩm  | **100** (SKU-003)                    |
| Thời trang | **90** (4 variants AT-001 cộng lại)  |


Nếu Thời trang hiện 4 rows thay vì 1 → group-by chưa đúng.
...
- **Variant — TC-04:** Report 1 (HN-01, Thống kê theo: Hàng hóa) hiện 4 rows cho AT-001 với `parentSku = AT-001`, `color` và `size` đúng từng variant
- **Variant — TC-04b:** Report 1 (HN-01, Thống kê theo: Mẫu mã) gộp 4 variants thành 1 row AT-001, nhập kỳ = 90
- **Variant — TC-04c:** Report 1 (HN-01, Thống kê theo: Nhóm hàng hóa) row Thời trang có nhập kỳ = 90 (tổng 4 variants)
