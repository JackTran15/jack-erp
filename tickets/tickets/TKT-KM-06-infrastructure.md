...
- [ ] `findActive` nạp đủ quan hệ con **không N+1 theo số lượng CTKM** (không có truy vấn nào bên trong vòng lặp `.map()` qua danh sách program). *(amdt KM-06, chốt sau khi implement)*: "1 lấy con" ban đầu viết như thể toàn bộ quan hệ con gộp vừa 1 câu SQL — thực tế con trải trên 6 bảng (`groups`/`lines`/`tiers`/`conditions`/`branches`/`customerGroups`) không thể gộp một câu mà không tạo cross-join `lines × tiers` theo từng group. Chốt: `IN (programIds)` theo từng bảng con, chạy song song qua `Promise.all` (6 câu, không phải 1) — hằng số theo số bảng, không phụ thuộc số program. Đây mới là bất biến thật cần giữ.
