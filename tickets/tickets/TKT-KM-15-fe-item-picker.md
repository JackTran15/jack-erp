...
Sửa các lưới dưới `apps/backoffice-web/src/pages/promotions/programs/ProgramFormPage/`:

| Lưới | Hình thức | Chế độ chọn |
| ---- | --------- | ----------- |
| `GoodsDiscountSection/GoodsDiscountGrid` | `ITEM_DISCOUNT` | `PRODUCT` hoặc `ITEM`, và **`CATEGORY`** khi `goodsDiscountScope = GROUP` |
| `TieredDiscountSection/ProductSelectionGrid` | `TIERED_DISCOUNT` | `PRODUCT` / `ITEM` / `CATEGORY` theo `tierTarget` |
| `GiftSection/GiftProductGrid` | `GIFT_ITEM` | `ITEM` |
| `BuyGetSection/BuyGetProductGrid` (×2: điều kiện mua + hàng được tặng) | `BUY_M_GET_N` | theo `buyGetPurchaseTarget` |
| `ApplicableGoodsTable` | điều kiện `SPECIFIC_QUANTITY` | `ITEM` |

Có thể phát sinh:
- `promotions/components/PromotionTargetPicker.tsx` — wrapper mỏng trên `ProductSelectDialog` chuẩn hóa `ProductSelectResult` → `PromotionLine`-shape, để 6 chỗ gọi không lặp code map.
